...
public class ExploratoriaRouteManager extends BaseRouteFleetManager {

	//Based on HMI , thanks to mod author for allowing of usage of HMIScavengerClasses
	protected StarSystemAPI system;

	public MarketAPI getCapital(){
		for (MarketAPI market : Misc.getFactionMarkets(Global.getSector().getPlayerFaction().getId())) {
			if(market.getPlanetEntity()!=null){
				if (market.getPlanetEntity().getMemory().is("$ashencapital:" + Global.getSector().getPlayerFaction().getId(), true)) {
					return market;
				}
			}
	...
	public ExploratoriaRouteManager(StarSystemAPI system) {
		super(1f, 14f);
		this.system = system;
	}
	...
	protected int getMaxFleets() {
		//if (true) return 0;
		MarketAPI capital = getCapital();
		if(capital!=null){
			if(capital.hasIndustry(AoDIndustries.EXPLORATORIA_CORPS)){
				float salvage = getVeryApproximateSalvageValue(system);
				return (int) (1 + Math.min(salvage* (capital.getSize()-2), 14));
			}
	...
	public MarketAPI pickSourceMarket() {
		...
		for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
			if (market.getFaction().isHostileTo(Global.getSector().getPlayerFaction().getId())) continue;
	...
	public CampaignFleetAPI spawnFleet(RouteData route) {
		...
		CampaignFleetAPI fleet = createExploratoriaScavenger(type, system.getLocation(), route, route.getMarket(), pirate, random);
		if (fleet == null) return null;;
		
		fleet.addScript(new ExploratoriaAssignmentAI(fleet, route, pirate));
		
		return fleet;
	}
	
	public static CampaignFleetAPI createExploratoriaScavenger(String type, Vector2f locInHyper, MarketAPI source, boolean pirate, Random random) {
		return createExploratoriaScavenger(type, locInHyper, null, source, pirate, random);
	}
	public static CampaignFleetAPI createExploratoriaScavenger(String type, Vector2f locInHyper, RouteData route, MarketAPI source, boolean pirate, Random random) {
...
//			combat += utility;
		...
		FleetParamsV3 params = new FleetParamsV3(
				...
				locInHyper,
				Global.getSector().getPlayerFaction().getId(), // quality will always be reduced by non-market-faction penalty, which is what we want
		...
		fleet.setFaction(Global.getSector().getPlayerFaction().getId(), true);
		fleet.setNoFactionInName(false);
		fleet.getMemoryWithoutUpdate().set(MemFlags.MEMORY_KEY_SCAVENGER, true);
		fleet.getMemoryWithoutUpdate().set("$Ashen",true);
